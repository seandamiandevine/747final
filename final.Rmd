...
nbins <- 10
beautybin <- as.numeric(cut(beauty, 10))
choiceodds <- tapply(choices, beautybin, function(x) sum(x==1)/sum(x==0))
choiceodds[choiceodds==Inf] <- NA
choiceoddsIdx <- 1:length(choiceodds)  
lmexp <- lm(choiceodds ~ 0 + I(choiceoddsIdx^2))
...
plot(x=beauty, y=choices, ylab='Probability of saying yes')
lines(simpredict)
for(b in 1:length(beauty)) {
  bmod <- b %% (length(beauty)/nbins)
  if(bmod == 0 | b == 1) abline(v = b, col='red', lty='dotted')
}
text(labels='d', 
     x=grconvertX(0.05, from="nfc", to="user"),
     y= grconvertY(0.94,  from="nfc", to="user"), 
...
# e. odds
plot(choiceodds, ylab = 'Odds Ratio (yeses/nos)', xlab = 'Beauty Bin')
lines(predict(lmexp))
text(labels='e', 
     x=grconvertX(0.05, from="nfc", to="user"),
     y= grconvertY(0.94,  from="nfc", to="user"), 
...
The logistic function^[$e$ here is Euleur's constant, which is approximately equal to 2.718282] squeezes the output from our linear model to be between between 0 and 1, which we use to represent probabilities of responses being equal to 1. As an input, it takes $\eta$, which in our regression framework is the predicted value of $y$ from our linear model (i.e., the result of $\beta_0 + \beta_1Beauty_i$). We can see what this function looks like for our data in **Figure 1c**. The characteristic *S-curve* better represents our earlier intuition: the better-looking someone is, the higher probability with which they'll get a date. 

While transforming our data using a logistic function fixes our the issue of impossible values on our regression line, we have lost the nice linear function we had in **Figure 1b**. As such, we cannot apply the (multilevel) *linear* techniques we have learned in class and previously in our statistical training. To recover our linear function (i.e., to *linearize* our logistic function), while maintaining the desirable propoerties from the logistic function, we can compute the **odds ratio of an event occuring**. Like probabilities, odds ratio are a continuous measure of likelihood. They are computed by dividing the number of times an event occurs by the number of times it does not: $\frac{\Sigma y=1}{\Sigma y=0}$. To return to our example, an odds ratio might represent how many times people say yes to a date relative to how many times they say no, depending on one's beauty. To compute this, we can create "bins" of beauty and compute the odds of saying yes in each bin, as visualised in **Figure 1d**. Remember, all we are doing here is counting the number of times people say "yes" or "no" in each bin and dividing these two values. If we visualize this, we can see that as beauty increases, the odds of saying yes to a date increase **exponentially**. Roughly, for a one-unit increase in beauty bin, the odds ratio of saying yes to a date increases by $\beta_0\beta_1^{beauty \ bin}$ (see **Figure 1e**). This is ideal for us, because it is well-known that to linearlize an exponential function, all one must do is take its (natural) logarithm. The problem is that now the x-axis has changed from a continuous beauty to beauty bins! 

Finally it is time to bring everything together. To take advantage of both the exponential function of odds *and* the probabilistic logistic function we've discussed, we can compute the log of the odds ratio for our the probability of our outcome:
...
Now, our outcome is no longer the probability that $y_i = 1$, as it was in our logistic function, nor is it the raw odds, but instead the **log odds of $y_i$ being equal to 1** (which in this case means saying "yes" to a date). This is simply the natural logarithm of the probability that $y_i = 1$ (which we got from our sigmoidal function earlier) over the predicted probability that $y_i \neq 1$. Therefore, we can easily move from log odds (LOR) to regular odds ratios (OR) to probability as follows:
...
All together, we can apply these steps to any binomial data to reach a linear model that captures the trends we are interested in (for instance, in **Figure 1f**. Namely, how the odds (which is equivalen to the probability, as we have seen) of one $y = 1$ changes depending on the value of a number of predictors. 

## Multilevel Logistic Models
Equation 3 can easily be exteded to multilevel data now that we have a linear function to work with. The key thing to remember is that the coefficients refer to an outcome that is transformed and not in the original metric (log odds instead of binary data). The equation for a $k$-predictor multilevel logistic model, with random intercepts and random slopes, is:

$$
\tag{`r  eqnum=eqnum+1; eqnum`}
log\left(\frac{p(y_{ij} = 1)}{p(1-y_{ij}= 1)}\right) = \\
\gamma_{00} + U_{0j} + R_{ij} + \sum_{h=1}^{k}\gamma_{h0}x_{hij} +  U_{hj}x_{hij}, \\
$$
...
In equation 5, I include a term for $R_{ij}$, but as we saw earlier, level-1 residuals in logistic models are not a free parameter to be estimated, but rather depend on $y_{ij}$, such that $R_{ij} = p(y_{ij}=1)\left(1-p(y_{ij}=1)\right)$.To understand this fully, let us return to traditional linear regression. Here, each observation has a distinct residual to the regression line. For instance, if we assume that someone's muscle tone predicts their beauty relatively well, but not perfectly, and we fit a (level-1) linear model to these data, we obtain a fitted regression equation of the form $Beauty_i = \beta_0 + \beta_1Muscle_i + \epsilon$. Critically, $\epsilon$ represents some error in the estimated value for $Beauty_i$ and its true value. These values are residuals and are assumed to be normally distributed $\mathcal{N}(0, \sigma^2)$. From this example, it is clear that underlying linear regression is a normal distribution, which has two estimated parameters: a mean, $\mu$ and variance, $\sigma^2$ (or $R_{ij}$ in the context of MLM).

In contrast, binary data does not assume the same underlying distribution. Rather than assuming a normal (a.k.a. Gaussian) distribution to describe our errors (i.e., a distribution with $mu$ and $\sigma^2$), we now assume a Bernoulli distribution, because our data can only take on binary values (0 and 1). Unlike Gaussian distributions, Bernoulli distributions only have one parameter, $\theta$: the probability of an event occuring, conditional on the predictors. Therefore, if we imagine the simplest Bernoulli example, where we have no predictors (i.e., a null model), then all of the information one could know about the distribution is contained within (1) the sample size and (2) $\theta$, which is directly observable in the data (e.g., the number of "yeses" to dates is plainly calculable). As a result, there is no error term, "$\epsilon$" to speak of in the same way we talk about it in a Gaussian distribution: as something that is added to or substracted from $\theta$. Rather the "variance" of a Bernouilli distribution, is entirely determined by $\theta$ according to the following equation: $var = \theta(1-\theta)$, which earlier I wrote as $p(1-p)$.^[To demonstrate why this equation is true, let $X$ be some binary variable and $\theta$ be the mean of the distribution. We begin by solving for $\theta(X^2)$ as follows: $\theta^2 = p(X=1) \times 1^2 + p(X=0)\times 0^2 = p(X=1)1 + p(X=0)0 = p(X=1) = \theta$. Following the standard equation for computing variance ($\sigma^2 = ((\Sigma X^2) / N ) - \mu^2$), keeping in mind $N$ is already captured by $\theta$ by virtue of it being a probability, we can thus compute the variance around $\theta$ as: $V(X) = \theta(X^2)-\theta^2 = \theta - \theta^2 = \theta(1-\theta)$. From this we can clearly see that the variance is just a restatement $\theta$.]. 

For this reason, the within-cluster residual term, $R_{ij}$, is usually dropped from logistic MLM equations and even excluded from the `lme4` output, as we will see. A particular quirk of the definition of variance in the logistic model that we have seen is that the variance about $R_{ij}$, is sometimes assumed to be random and independent, with mean 0 and a fixed variance of $\frac{\pi^2}{3}$. I'll explain when this assumption is appropriate and where it comes from in a later section discussing the calculation of an ICC and residual variance in logistic multilevel models, but for those who are interested, see Snijders & Bosker (2012, section 17.3.3-4). 

Save for $R_{ij}$, the remaining parameters ($\gamma, \tau^2$) must be estimated. How `lme4` estimates these parameters is beyond the scope of the current tutorial, but suffice it to say that we will use the default Laplace Approximation method for the remainder of this tutorial. For more information about approximation methods for hiearchical data, see Raudenbush et al. (2000). 
...
The data for this tutorial comes from an experiment I ran earlier this year through Amazon Mechnical Turk (AMT). The data stored in `effort_progress_choice.csv` describes 60 participants' choices in a cognitive experiment about mental effort preference when progress feedback was present or not. 

### Study Background
Past work has shown that people find cognitive effort aversive and, when given the option, they will avoid it (Kool et al., 2010). However, some activities (e.g., puzzle-solving, Suduko, building furniture) are performed by humans *because* they are effortful, not despite the fact (Inzlicht et al., 2018). In this study, we wanted to see whether **progress** was a feature of an effortful task that motivated people to freely choose harder mental work. The main question was whether the presence of progress feedback in a mentally demanding task would make people choose to work mentally harder. 
...
To do so, we can assign numeric labels to categorical predictors by using a *coding scheme*. In our case, we are interested in how effort choices within a certain deck pairing differ from indifference (50% responding) and less interested in how these deck pairings differ from each other. For this reason, we will use a **dummy coding scheme**, where a value of 1 represents that level of the category (that deck pairing) is present and 0 represents when it is not. R does this for us automatically when our variable is a factor. We can access this default dummy coding scheme using by using the contrast function. 
...
$$
$\gamma_{00}$ represents the estimated log odds that participants chose the high demand task (when `effchoice` = 1) overall. This model will also estimate $\tau^2_0$, which will be the variance about $\gamma_{00}$ per participant. In other words, how much each participant's estimated probability of choosing high demand tasks differs from $\gamma_{00}$ (in log odds). Recall, $R_{ij}$ is actually fixed in this case and $\sigma^2 = p(effchoice_{ij})(1-p(effchoice_{ij})$. $i$ is for the $i^\mathrm{th}$ trial for the $j^\mathrm{th}$ participant.
...
This is telling us that the model predicts that, on average, people chose the high demand task about 41% of the time. To be sure that we didn't make a mistake, we can check the same probability in our empirical data.
...
Here we can see that the confidence interval for the log odds of choosing the high demand task does not cross zero: $\mathrm{95\% \ CI} = [-0.50, -0.25]$. Applying the same computations as earlier, this is the same as saying that the CI for choosing high demand decks overall is $[38\%, 44\% ]$. These results support our first hypothesis and show that people avoid high cognitive effort tasks more than we would expect from indifference (50% responding). We can see a summary of these results in **Figure 3**.
...
This approach suggests that 5% of the variance in participants' overall effort preferences stems from individual differences between participants.
...
```{r u0js plot, echo=F, fig.width=5, fig.height=6, fig.align='center', fig.cap='Individual variation for 6 participants in demand preference (H1).'}
...
# Bonus: if you fit the random slopes model with glmer directly, 
# without constraining the covariance matrix, it will fail to converge.
...
By taking the average value of the residuals in each of these bins, we can detect if there is any systematic variation in the residuals across the predicted values. In other words, if our residuals are truly randomly distributed (as we assumed them to be when fitting the model), we expect that the average of the deviance residuals do not vary systematically according to the predicted values. 
...
Hopefully, this tutorial has been successful in demonstrating how to implement, interpret, and utilise logistic MLMs for testing your hypotheses. As I mentioned, I did not cover everything to do with the current dataset, for the sake of space and for pedagogical reasons (i.e., showing how to idenitify misspecified models through binned residual plots). Specifically, I only looked at a one predictor model (`prog_type` in `m1`), beyond the null model (`m0`). In the full analyses for this project, I also consider the role of `effort_type` and its interaction with `prog_type`. If these results are of interest, and you believe the current tutorial isn't too long to include them, please let me know in the Discussion Board and I can add them to the final tutorial. If you are interested in anything to do with the project or have any questions, don't hesitate to [email me](mailto:seandamiandevine@gmail.com). 
...
# Revision Letter
Peers provided helpful comments on what to improve in the tutorial. I aimed to incorporate their feedback as best as possible. 

1. **Darius**: 

  a. *"When explaining the steps between raw predictions, log odds, and regular odds, are there any visualizations that would help illustrate the differences between these ways of representing the outcome?"*

    This information was added to the **Logistic Regressions, Probabilities, and Odds Ratios: Basics of Logistic Regressions** section. Specifically, two additional subfigures were added to **Figure 1** (**d** and **e**) to illustrate how odds ratio are computed and their exponential nature. These additional subfigures come as the direct result of a back-and-forth Darius and I had in the discussion board that aimed to determine the most accessible way to present this information in text-format. From these figures, the remainder of the explanation is relatively straightforward, building on the section's narrative of transforming binary data in a variety of ways to obtain desirable (i.e., linear) models. 
  
   b. *"It could be clarified at the beginning of the model construction sections that effort_type will not feature in the demo, to give a better idea of the scope upfront. I was expecting it to come up, and so it took some extra effort to keep it in mind while reading."*
  
    This issue was likely the result of the following sentence, which was left over from a previous version of the tutorial: "Specifically, we will try to model whether participants' were more likely to choose one type of deck over another depending on **(a) the demand pairing (how hard each available deck was)** and (b) the progress pairing (whether one, both, or none of the decks gave progress feedback) on a given trial." To correct for this, the bolded section of the sentence was removed, as well as any mention of the demand pairing variable (`effort_type`) in the context of hypothesis testing, to avoid confusion. 
  
  c. *"On page 12, should “some value U0j” instead use a lower-case “u”, since U0j is a random variable?"*

    The tutorial has been double-checked to ensure that all $U_{0j}$ are consistenly upper-case. Personally, I find this more intuitive for the intended audience, as this is the notation we used in class throughout the semester. 


2. **Anna**: 
  a. *"I've been wondering why it is tricky to calculate the within-cluster variance in logistic models? Why is the p(1-p) calculation of variance impossible/inaccurate? I know that you explained this in two places in the text, so I could be missing a key point, but would be great to hear again."*

    Additional information on level-1 logistic residuals has been added to the **Logistic Regressions, Probabilities, and Odds Ratios: Multilevel Logistic Models** section to address this point. Specifically, these new paragraphs address why level-1 variance in logistic models is determined by $p(y_i=1)$ in a more fine-grained manner. In-text, this is explained verbally, with analogies to linear regression for ease of comprehension. Additionally, in footnote 5, the same point is made computationally using simple-to-follow equations that should be familiar to students at this level (e.g., to compute the variance of a variable around its mean).
    
3. **Sophie**: 
  a. *"In testing your 2nd hypothesis, I wondered if you could elaborate more on the random effects (page 18).  When specifying the random slopes in "m1.slopes.nocov" you had entered "0 + prog_type", but the output shows a different equation. I'm just wondering what's going on there."*
    
    Footnote 12 now addresses the discrepancy between the inputted model equation and the R output model equation. Specifically, the console notation is just a quirk of `afex`, but the interpretation for all effects remains the same. Hopefully this clears up confusion for any particularly sharp-eyed readers 
